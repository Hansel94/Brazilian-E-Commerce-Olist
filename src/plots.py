...
def plot_freight_value_weight_relationship(df: DataFrame):
    ...
    """
    # plot freight value weight relationship using seaborn scatterplot.
...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    """
    # plot order amount per day with holidays using matplotlib.
    plt.figure(figsize=(12,6))
    plt.plot('date', 'order_count', data=df, color='green')
    [ plt.axvline(x=date, linestyle=':') for date in df[df['holiday']]['date'] ]
    plt.show()
