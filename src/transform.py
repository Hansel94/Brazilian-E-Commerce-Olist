...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    # Merge items, orders and products tables
    ...
    aggregations = delivered.groupby('order_id').agg('sum')[['freight_value', 'product_weight_g']].reset_index()

    # This will return the result from `aggregations` variable with the corresponding name and format.
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    filtered_dates = orders[orders['order_purchase_timestamp'].dt.year == 2017]['order_purchase_timestamp']

    # Counting the orders per day.
    order_purchase_ammount_per_date = filtered_dates.dt.date.value_counts()

    # Creating a dataframe with the result. Assign it to `result_df` variable.
    result_df = DataFrame({'order_count': order_purchase_ammount_per_date.values, 'date': pd.to_datetime(order_purchase_ammount_per_date.index)})
